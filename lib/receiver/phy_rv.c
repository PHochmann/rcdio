...
#define ICP1            0 // Pin for input capture interrupt
const uint16_t TICKS_PER_PULSE = (F_CPU / PRESCALER) * (PULSE_US / 1000000); // 6

#define BUFFER_SIZE     100 // When bigger than 255, some data types have to be changed
...
{
    static uint16_t high_pulses = 0;
    static uint16_t low_pulses = 0;
        ...
        if (is_listening)
        {
            ...
            else
            {
                ...
                if (curr_bit == 8)
                {
                    ...
                    if (((tentative_head + 1) % BUFFER_SIZE) == tail)
                    {
                        is_listening = false;
                        return;
                    }
